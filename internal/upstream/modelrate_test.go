...
func TestModelRateIsModelRateLimit(t *testing.T) {
	yes := []string{
...
// TestParseSoftRateResetRejectsNonModelLimit：ParseRateReset 只负责解析重置墙钟；
// 是否模型级限流由 IsModelRateLimit 单独判定（调用方组合使用）。
// 无时间/非法时间仍必须拒绝。
